...
const EditPremiumPostForm = () => {
  ...
  const [body, setBody] = useState(() => premium.body);
  ...
  const formik = useFormik({
    initialValues: {
      ...
      homeTeam: premium.homeTeam,
      awayTeam: premium.awayTeam,
    ...
    onSubmit: async (values, { setSubmitting }) => {
      ...
      Object.keys(values).forEach((key) => {
        if (values[key] !== premium[key]) {
          isChanged = true;
        }
      });

      if (body !== premium.body) {
        isChanged = true;
      }
      ...
      try {
        ...
        await firestore.collection('premium').doc(premium.id).update(update);
        toogleModal({
          open: false,
          component: '',
        });
...
export default EditPremiumPostForm;
