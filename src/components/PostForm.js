...
const PostNewsForm = () => {
  const { modal, toogleModal, toogleToast, leagues, profile } = useContext(
    AppContext
  );

  const collection = modal.component === 'Make News Post' ? 'news' : 'forum';
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { setSubmitting }) => {
      ...
      try {
        ...
        await firestore.collection(collection).add(update);
        toogleModal({
          open: false,
          component: '',
        });
